...
//    }
    ...
    @GetMapping("/alertPatient/{providerID}")
    @ResponseStatus(HttpStatus.CONFLICT)
    @CircuitBreaker(name ="alert",fallbackMethod = "circuitAlertFromHealthCareService")
    @Retry(name = "alert")
    public String alertFromHealthCareService( @PathVariable("providerID") int providerID)
    {
        return alertService.alertFromHealthCareService(providerID);

    }

    public String circuitAlertFromHealthCareService(@PathVariable("providerID") int providerID,RuntimeException runtimeException)
    {
